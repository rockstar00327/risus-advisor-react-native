...
const Post = ({ data, isFocused }) => {
  ...
  const handleGetReposts = async () => {
    ...
    try {
      // console.log("Calling getReposts..."); // Debug log
      await getReposts();
      // console.log("getReposts completed"); // Debug log
    } catch (error) {
      console.error("Error in handleRepostPress:", error);
    }
  };
  ...
  const scrollToTop = () => {
    ...
    // Reset post height to full size if needed
  ...
  const handleRepostSelect = (repost) => {
    ...
    scrollToTop(); // Refresh reposts for the new post
  };
  ...
  // This will run every time the post becomes focused
  useEffect(() => {
    if (isFocused) {
      ...
      // Optional: fade out when losing focus
      fadeAnim.setValue(0); // Ensure it's visible before fading out
      Animated.timing(fadeAnim, {
        toValue: 0,
        duration: 10,
        useNativeDriver: true,
  ...
  return (
    ...
    >
      {isScrolled && <View style={styles.threadConnector} />}
      {/* Main Post */}

      <Animated.View style={[styles.carouselContainer, { height: postHeight }]}>
            ...
            <Pressable onPress={handlePostPress}>
              ...
              >
                ...
                >
                  <TouchableOpacity
                    onPress={() =>
                      navigation.navigate("HostProfile", { user: data?.user })
                    }
                    style={styles.header}
                  >
                    <Image
                    ...
                    <View style={styles.userInfo}>
                      ...
                      <View style={styles.usernameTimeContainer}>
                        <Text style={styles.timeAgo}>
                          ...
                          {/* Splits at '@' and takes the first part (username) */}
                        </Text>
                  ...
                  <View style={styles.stats}>
                    <View style={styles.leftStats}>
                      ...
                      >
                        ...
                        <Text style={styles.statText}>
                          ...
                          {currentPost?.total_comments >= 1000
                            ? (currentPost?.total_comments / 1000).toFixed(
                                currentPost?.total_comments % 1000 === 0 ? 0 : 1
                              ) + "K"
                      ...
                      >
                        ...
                        {/* <Text style={styles.statText}>
                      {" "}
                      {currentPost?.total_reposts >= 1000
      ...
      <View style={styles.repostsContainer}>
        ...
        <Animated.View style={[animatedStyle]}>
          {loading ? (
            ...
            >
              {!isScrolled && <BlurViewButton scrollY={scrollY} />}
              {showOverlay && (
                <Animated.View
...
const styles = StyleSheet.create({
  ...
  threadConnector: {
    position: "absolute",
    left: width / 2.1, // Half of avatar width (48/2) to center it
    top: -30,
    bottom: 150,
    width: 1,
    backgroundColor: "#757575ff", // Light gray color
    zIndex: 0,
  },
  repostsContainer: {
    marginTop: 20,
  ...
  scrolledContainer: {
    // height: 200, // Reduced height when scrolled
    paddingTop: 20,
  },
  scrolledCarousel: {
    //height: 200, // Reduced height when scrolled
